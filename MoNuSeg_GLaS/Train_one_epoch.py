...
def print_summary_old(epoch, i, nb_batch, loss, loss_name, batch_time,
...
def train_one_epoch(loader, model, criterion, optimizer, writer, epoch, lr_scheduler, fold, kfold, logger):
    ...
    for i, (sampled_batch, names) in enumerate(loader, 1):
        ...
        if model.training:
            ...
            preds4,preds3,preds2,preds1  = model(images)
            out_loss4 = criterion(preds4, masks.float())  # Loss4
            out_loss3 = criterion(preds3, masks.float())  # Loss3
            out_loss2 = criterion(preds2, masks.float())  # Loss2
            out_loss1 = criterion(preds1, masks.float())  # Loss1
            out_loss = out_loss4 +out_loss3 + out_loss2 + out_loss1
        else:
            preds4  = model(images)
            out_loss4 = criterion(preds4, masks.float())  # Loss4
            out_loss = out_loss4
