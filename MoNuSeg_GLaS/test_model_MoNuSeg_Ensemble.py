...
def show_image_with_dice(predict_save, labs, save_path):
    ...
    numerator = 2 * np.sum(tmp_lbl * tmp_3dunet)

    denominator = (np.sum(tmp_lbl) + np.sum(tmp_3dunet) + 1e-5)
...
def vis_and_save_heatmap(ensemble_models, input_img, img_RGB, labs, lab_img, vis_save_path):
    ...
    bb=torch.cat(output_save_tmp,dim=1)
    out_aa = bb.mean(1)
    pred_class_tmp = torch.where(out_aa>0.5,torch.ones_like(out_aa),torch.zeros_like(out_aa))
    predict_save_tmp = pred_class_tmp[0].cpu().data.numpy()
    dice_pred_tmp, iou_tmp = show_image_with_dice(predict_save_tmp, labs, save_path=vis_save_path+'_predict'+model_type+'.jpg')
